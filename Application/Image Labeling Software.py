...
class MainApplication(Tk.Tk):
	...
	def __init__(self):
		...
		self.datetime_ls = []
		self.lbl_ls = ['person', 'home', 'playground', 'void_deck', 'park', 'public_space', 'supermarket', 'market',
		               'food_court', 'shop', 'mall', 'hospital', 'clinic', 'community_center', 'senior', 'religious',
		               'transaction_ser', 'fitness', 'bus_stop', 'mrt', 'walkway', 'pedestrian_crossing',
		               'cycling_path', 'street_lights', 'traffic_lights', 'street_signs', 'trees', 'furniture',
		               'stairs', 'ramps', 'walk', 'cycle', 'bus', 'train', 'car', 'drive', 'sit', 'chat', 'eat',
		               'shopping', 'run', 'exercise', 'not_useful']
		...
		self.b1 = Tk.Button(self.left_frame, text = "Select", bg = "SkyBlue1", command = self.open_photo)\
			.grid(row = 0, column = 2, padx = 2)
		self.b2 = Tk.Button(self.left_frame, text = "Update", bg = "Olivedrab1", command = self.update)\
			.grid(row = 0, column = 3, padx = 2)
		self.b3 = Tk.Button(self.left_frame, text = "Show labels", command = self.show_lbl)\
			.grid(row = 0, column = 4, padx = 2)
		self.b4 = Tk.Button(self.left_frame, text = "Clear checks", command = self.clear_check)\
			.grid(row = 0, column = 5, padx = 2)
		self.b5 = Tk.Button(self.left_frame, text = "Clear attribute", command = self.clear_attribute)\
			.grid(row = 0, column = 6, padx = 2)
		
		self.var_msg = Tk.StringVar()
		self.msg_lbl = Tk.Label(self.left_frame, textvariable = self.var_msg, font = ("Helvetica", 12))\
	...
	def gen_tag_labels(self):
		...
		var = Tk.IntVar()
		void_deck = Tk.Checkbutton(location_tab, text = "Void deck", variable = var)
		self.cb_ls.append((void_deck, var))
		var = Tk.IntVar()
		...
		publicgoods = Tk.Checkbutton(location_tab,
		                             text = "Public furniture (e.g. benches, tables, places related to to "
		                                    "sitting/resting)",
		                             variable = var)
	...
	def open_photo(self):
		...
		for file_name in os.listdir(self.path.get()):
			...
			if file_name.endswith(".jpg"):
				self.image.insert(0, Image.open(os.path.join(self.path.get(), file_name)).resize(
					(int(width / no_row - 60), 145)))
	...
	def update(self):
		...
		if len(self.file_chosen_ls) < 1:
			...
			try:  # if file not exist in database, add new row
				...
				self.run_query(
					"UPDATE attributes SET person=0,home=0,playground=0,void_deck=0,park=0,public_space=0,"
					"supermarket=0,market=0,food_court=0,shop=0,mall=0,hospital=0,clinic=0,community_center=0,"
					"senior=0,religious=0,transaction_ser=0,fitness=0,bus_stop=0,mrt=0,walkway=0,"
					"pedestrian_crossing=0,cycling_path=0,street_lights=0,traffic_lights=0,street_signs=0,trees=0,"
					"furniture=0,stairs=0,ramps=0,walk=0,cycle=0,bus=0,train=0,car=0,drive=0,sit=0,chat=0,eat=0,"
					"shop=0,run=0,exercise=0,not_useful=0 WHERE file = (?)",
				...
				for j in range(1, len(self.file_chosen_ls)):
					try:  # if file not exist in database, add new row
						...
						self.run_query(
							...
							"supermarket=0,market=0,food_court=0,shop=0,mall=0,hospital=0,clinic=0,community_center=0,"
							"senior=0,religious=0,transaction_ser=0,fitness=0,bus_stop=0,mrt=0,walkway=0,"
							"pedestrian_crossing=0,cycling_path=0,street_lights=0,traffic_lights=0,street_signs=0,"
							"trees=0,furniture=0,stairs=0,ramps=0,walk=0,cycle=0,bus=0,train=0,car=0,drive=0,sit=0,"
							"chat=0,eat=0,shop=0,run=0,exercise=0,not_useful=0 WHERE file = (?)",
	...
	def get_datetime(self, f):
		...
		try:
			datetime = name[0:4] + "-" + name[4:6] + "-" + name[6:8] + " " + name[9:11] + ":" + name[11:13] + ":" \
			           + name[13:15]
		except:
			datetime = None
		return datetime
